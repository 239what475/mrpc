...
private:
    ...
    for (const auto &method : service.methods) {
      ...
      for (const auto &param : method.request_params) {
        output << "\t" << capitalize(param.name) << " "
               << generateGoType(param.type) << " `json:\"" << param.name
               << "\"`\n";
      }
      output << "}\n\n";

      // 生成请求ToString方法
      output << "func (r *" << method.name
             << "Request) ToString() (string, error) {\n";
      ...
      // 生成请求FromString方法
      output << "func (r *" << method.name
             << "Request) FromString(data string) error {\n";
      ...
      // 生成响应ToString方法
      output << "func (r *" << method.name
             << "Response) ToString() (string, error) {\n";
      output << "\tdata, err := json.Marshal(r)\n";
      output << "\tif err != nil {\n";
      output << "\t\treturn \"\", err\n";
      output << "\t}\n";
      ...
      // 生成响应FromString方法
      output << "func (r *" << method.name
             << "Response) FromString(data string) error {\n";
      output << "\treturn json.Unmarshal([]byte(data), r)\n";
      output << "}\n\n";
    }
  }
  ...
  void generateClient() override {
    ...
    // 生成构造函数
    output << "func New" << service.name << "Client(s string) *" << service.name
           << "Client {\n";
    ...
    for (size_t i = 0; i < service.methods.size(); i++) {
      ...
      output << "\treturn response." << capitalize(first_response.name)
             << ", err\n";
      output << "}\n\n";
      ...
      output << "\treturn h.client.AsyncSend(" << service.name
             << "_method_names[" << std::to_string(i) << "], request)\n";
      output << "}\n\n";
      ...
      output << "\th.client.CallbackSend(" << service.name << "_method_names["
             << std::to_string(i)
             << "], request, response, func(err error) {\n";
      output << "\t\tcallback(response." << capitalize(first_response.name)
             << ", err)\n";
      output << "\t})\n";
      output << "}\n\n";
    }
    ...
    if (service.methods.size() > 1) {
      output
          << "func (h *" << service.name
          << "Client) Receive(key string, methodIndex int) (string, error) {\n";
      ...
      output << "\tdefault:\n";
      output << "\t\treturn \"\", fmt.Errorf(\"unknown method index: %d\", "
                "methodIndex)\n";
      output << "\t}\n";
      output << "}\n\n";
    } else {
      const auto &method = service.methods[0];
      output << "func (h *" << service.name
             << "Client) Receive(key string) (string, error) {\n";
      output << "\tresponse := &" << method.name << "Response{}\n";
      output << "\terr := h.client.Receive(key, response)\n";
      auto first_response = method.response_params[0];
      output << "\treturn response." << capitalize(first_response.name)
  ...
  void generateService() override {
    ...
    // 生成服务构造函数
    output << "func New" << service.name << "Service() *" << service.name
           << "Service {\n";
    output << "\tsvc := mrpc.NewMrpcService(\"" << yaml_filename << "."
           << service.name << "\")\n";
    ...
    for (size_t i = 0; i < service.methods.size(); i++) {
      ...
      output << "\t\tfunc() mrpc.Parser { return &" << method.name
             << "Request{} },\n";
      output << "\t\tfunc() mrpc.Parser { return &" << method.name
             << "Response{} },\n";
    ...
    output << "func New" << service.name << "Server(addr string) *"
           << service.name << "Server {\n";
...
public:
  ...
  bool generate(const std::string &output_dir) override {
    ...
    std::string go_file(output_dir + "/" + yaml_filename + ".mrpc.go");
    std::ofstream out_file_1(go_file);
    if (!out_file_1) {
      std::cerr << "Failed to open output file: " << go_file << std::endl;
      return false;
    }
