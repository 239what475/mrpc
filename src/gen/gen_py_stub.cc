...
private:
    ...
    void generateStructs() override {
        for (const auto& method : service.methods) {
            ...
            // 生成响应类
            output << "class " << method.name << "Response(mrpc.ParseFromJson):\n";
    ...
    void generateService() override {
        // 生成类定义
        output << "class " << service.name << "Service(mrpc.Service):\n";
        ...
        // 注册所有方法的处理函数
        for (size_t i = 0; i < service.methods.size(); ++i) {
            output << "        self.register_handler(\n";
            output << "            " << service.name << "_METHOD_NAMES[" << i << "],\n";
            output << "            self._" << toLower(service.methods[i].name) << "_handler\n";
            output << "        )\n";
        }
        output << "\n";
        ...
        for (const auto& method : service.methods) {
            // 生成处理函数
            output << "    def _" << toLower(method.name) << "_handler(self, request_data: str) "
                  << "-> tuple[str, Exception | None]:\n";
            output << "        request = " << method.name << "Request(";
            
            // 为请求参数生成默认值
            for (size_t i = 0; i < method.request_params.size(); ++i) {
                if (i > 0) output << ", ";
                auto [_, default_value] = getPythonTypeAndDefault(method.request_params[i].type);
                output << default_value;
            }
            output << ")\n";
            
            output << "        request.fromString(request_data)\n";
            output << "        response = " << method.name << "Response()\n";
            output << "        err = self." << method.name << "(request, response)\n";
            output << "        if err:\n";
            output << "            return \"\", err\n";
            output << "        return response.toString(), None\n\n";
        }

        // 为每个方法生成抽象方法
        for (const auto& method : service.methods) {
            output << "    def " << method.name << "(self, request: " << method.name 
                  << "Request, response: " << method.name << "Response) -> Exception | None:\n";
            output << "        raise NotImplementedError(\"" << method.name 
